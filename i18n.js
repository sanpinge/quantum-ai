...
const I18N = {
  ...
  'kline.live':     { zh: '实时策略信号 · V2 引擎', en: 'Live signals · V2 engine', ja: 'リアルタイム · V2エンジン', ko: '실시간 신호 · V2 엔진' },
  'live.title':     { zh: '实时策略引擎 · V2', en: 'Live Strategy Engine · V2', ja: 'リアルタイム戦略 · V2', ko: '실시간 전략 엔진 · V2' },
  'live.pos':       { zh: '当前持仓', en: 'Position', ja: '現在ポジション', ko: '현재 포지션' },
  'live.equity':    { zh: '实时权益', en: 'Live Equity', ja: 'リアルタイム資産', ko: '실시간 자산' },
  'live.wr':        { zh: '胜率', en: 'Win Rate', ja: '勝率', ko: '승률' },
  'live.trades':    { zh: '累计信号', en: 'Total Signals', ja: '累計シグナル', ko: '누적 신호' },
...
// ═══ 真信号解锁 (连钱包签名 · 后端验签查入金 ≥$1000) ═══
const SIGNAL_BASE = 'https://quantum-scanner.bard999.workers.dev/api/signal';
function getUnlockAuth() {
  try {
    const a = JSON.parse(localStorage.getItem('quantumUnlock') || 'null');
    if (a && a.ts && Date.now() - a.ts < 24 * 3600 * 1000) return a;   // 签名 24h 有效
  } catch (e) {}
  return null;
}
function signalUrl() {
  const a = getUnlockAuth();
